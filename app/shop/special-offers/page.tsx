...
// Import the special offers products
import { specialOffersProducts } from "@/lib/special-offers-data"
...
export default function SpecialOffersPage() {
  ...
  // Get all unique categories from special offers products
  const categories = Array.from(new Set(specialOffersProducts.map((product) => product.category))).map((category) => ({
    id: category,
    name: category.charAt(0).toUpperCase() + category.slice(1),
  }))

  // Get all unique tags from special offers products
  const allTags = Array.from(new Set(specialOffersProducts.flatMap((product) => product.tags || [])))
  ...
  useEffect(() => {
    // Apply filters
    let result = [...specialOffersProducts]
    ...
    <main className="min-h-screen">
      ...
      <div className="container mx-auto px-4 py-8">
        ...
        <div className="relative h-[300px] md:h-[400px] mb-8 rounded-lg overflow-hidden">
          <FallbackImage
            src="https://images.unsplash.com/photo-1490481651871-ab68de25d43d?ixlib=rb-1.2.1&auto=format&fit=crop&w=1200&q=80"
            alt="Special Offers"
            fill
          ...
          <div className="absolute inset-0 bg-black/30 flex flex-col items-center justify-center text-white p-4">
            <h1 className="text-4xl md:text-5xl font-bold mb-4 text-center">Special Offers</h1>
            <p className="text-lg md:text-xl max-w-2xl text-center mb-6">
              Limited time discounts on selected items - Don't miss out!
            </p>
