...
// Import the summer collection products
import { summerCollectionProducts } from "@/lib/summer-collection-data"
...
export default function SummerCollectionPage() {
  ...
  // Get all unique categories from summer collection products
  const categories = Array.from(new Set(summerCollectionProducts.map((product) => product.category))).map(
    (category) => ({
      id: category,
  ...
  // Get all unique tags from summer collection products
  const allTags = Array.from(new Set(summerCollectionProducts.flatMap((product) => product.tags || [])))

  useEffect(() => {
    // Simulate API call
    setTimeout(() => {
      setProducts(summerCollectionProducts)
      setLoading(false)
    }, 1000)
  }, [])
  ...
  useEffect(() => {
    // Apply filters
    let result = [...summerCollectionProducts]
    ...
    <main className="min-h-screen">
      ...
      <div className="container mx-auto px-4 py-8">
        ...
        <div className="relative h-[300px] md:h-[400px] mb-8 rounded-lg overflow-hidden">
          <FallbackImage
            src="https://images.unsplash.com/photo-1473496169904-658ba7c44d8a?ixlib=rb-1.2.1&auto=format&fit=crop&w=1200&q=80"
          ...
          <div className="absolute inset-0 bg-black/30 flex flex-col items-center justify-center text-white p-4">
            <h1 className="text-4xl md:text-5xl font-bold mb-4 text-center">Summer Collection 2025</h1>
            <p className="text-lg md:text-xl max-w-2xl text-center mb-6">
              Discover our latest summer styles and trends for the upcoming season
            </p>
        ...
        <h1 className="text-3xl font-bold mb-6">Summer Collection</h1>
